...
@section('content')
    ...
    <div class="col-lg-12 text-right">
        <a href="{{ route('certification.category.create') }}" class="btn btn-primary">
            ...
            <tbody>
                @if(count($category))
                    @foreach($category as $c)
                        <tr>
                            <td>{{ $c->id }}</td>
                            <td>{{ $c->certification->certification_name_en }}</td>
                    ...
                    @endforeach
                @else
                    <tr><td colspan="5" class="text-center">Sem Categorias.</td></tr>
                @endif
            </tbody>
        </table>
    </div>
